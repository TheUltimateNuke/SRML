...
namespace SRML.SR.SaveSystem.Data.Partial
{
    ...
    {
        private readonly List<PartialData> partialDatas = new List<PartialData>();
    ...
    internal abstract partial class PartialData
    {
        ...
        static readonly Dictionary<Type, PartialDataCreatorDelegate> partialDataCreators = new Dictionary<Type, PartialDataCreatorDelegate>();
        ...
        {
            RegisterPartialData(typeof(T), () => creator());
        }
        ...
        {
            return new CompoundPartialData<T>(partialDataCreators.Where((x) => x.Key.IsAssignableFrom(typeof(T))).Select((x) => x.Value()));
        }
